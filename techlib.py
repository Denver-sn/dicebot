...
def elimina_lista_maggiori(lista,n=0): #Elimina gli elementi maggiori di un numero da una lista di numeri
    nlista = []
    for x in range(len(lista)):
        lista[x] = int(lista[x])
        if lista[x] < n:
...
def elimina_lista_minori(lista,n=0): #Elimina gli elementi minori di un numero da una lista di numeri
    nlista = []
    for x in range(len(lista)):
        lista[x] = int(lista[x])
        if lista[x] > n:
...
def index(testo,carattere): #Ottiene la posizionee del carattere nel testo
    indice = []
    for x in range(len(testo)):
        if testo[x] == carattere:
            indice.append(x)
...
def ordina(lista): #Ordina una lista numerica dal minore al maggiore
    nlista = []
    for x in range(len(lista)):
...
def pari(numero): #Verifica se un numero è pari o meno
    numero = int(numero)
    if numero % 2 == 0:
        return True
    else:
...
def sostituisci_posizione(testo,carattere,posizione): #Pone un dato carattere in una data posizione in un testo
    testo = list(testo)
    testo[posizione] = carattere
    return delistizza(testo)

def superparole(text,lista): #Divide un testo in parole, usando come discriminante tutti i valori del dizionario
    text = str(text)
    x = 0
    t = ''
    parole = []
